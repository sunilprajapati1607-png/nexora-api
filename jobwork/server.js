...
if (process.argv[1] && /server\.js$/.test(process.argv[1])) {
  ...
  }).listen(PORT, () => { if (!keySource()) console.log('No Gemini key found. Candidates: ' + keyHint()); console.log('Nexora Jobwork AI listening on ' + PORT + ' — AI ' + (keySource() ? 'on (key from ' + keySource() + ')' : 'OFF (no GEMINI_API_KEY)')); });
}
